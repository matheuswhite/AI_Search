...
{
    return getId() == new EightPuzzleId("12345678x");
}
...
std::vector<Operator*> EightPuzzle::EightPuzzleState::getAllowedOperators()
{
    ...
    switch (pos.first) {
    case 0:
        operatorAllowed.erase(std::find(operatorAllowed.begin(), operatorAllowed.end(), up), operatorAllowed.end());
        break;
    case 2:
        operatorAllowed.erase(std::find(operatorAllowed.begin(), operatorAllowed.end(), down), operatorAllowed.end());
        break;
    default:
        break;
    }
...
Id* EightPuzzle::EightPuzzleState::applyOperator(Operator* op)
{
    ...
    if (op == new EightPuzzleOperator("up"))
    {
        numberPos = std::pair<int, int>(blankPos.first - 1, blankPos.second);
    }
    else if (op == new EightPuzzleOperator("down"))
    {
        numberPos = std::pair<int, int>(blankPos.first + 1, blankPos.second);
    }
    else if (op == new EightPuzzleOperator("left") == 0)
    {
        numberPos = std::pair<int, int>(blankPos.first, blankPos.second - 1);
    }
    else if (op == new EightPuzzleOperator("right"))
    {
        numberPos = std::pair<int, int>(blankPos.first, blankPos.second + 1);
    }
